...
export class VmForm extends ViewModel {
    constructor(bands, values) {
        super();
        /*
        protected fields: Field[];
        protected arrs: ArrFields[];
        protected onFieldsInputed: (values:any) => Promise<void>;
        //protected vmUsq: VmApi;
    
        init({fields, arrs, ui, readOnly}:VmFormOptions) {
            this.fields = fields;
            this.arrs = arrs;
        ...
        protected buildControl(fieldUI: FieldUIX, formValues:FormValues):VmControl {
            ...
            let tuid = field._tuid; //this.vmUsq.getTuid(tuidName);
            let pickerConfig: PickerConfig = {
                picker: undefined,          // TypeVmTuidPicker;
                row: undefined,             // RowContent
            };
            return new VmTuidControl(fieldUI, formValues, tuid, JSONContent, pickerConfig)
        }
        ...
        private buildBands(ui: FormUI) {
            ...
            if (ui !== undefined) {
                let {className, bandUIs} = ui;
                cn = className;
                bs = bandUIs;
            }
            this.ui = {
                className: cn || defaultClassName,
                bandUIs: bs === undefined?
                    this.buildOnFly() :
        ...
        showBands(fieldNames:string[], submitCaption?:any, onSubmit?:(values:any)=>Promise<void>) {
            ...
            let {bandUIs, visibleBands} = this.ui;
            visibleBands.splice(0, visibleBands.length);
            if (fieldNames === undefined) {
                visibleBands.push(...bandUIs);
                return;
            }
            for (let b of bandUIs) {
                switch (b.type) {
                    case 'fields':
        ...
        private buildFromBands(bandUIs:BandUI[], formValues:FormValues, fields:Field[]): BandUIX[] {
            ...
            for (let bandUI of bandUIs) {
                ...
                let arrBands = (bandUI as ArrBandUI).bandUIs;
        ...
        private buildArrBand(vBands:BandUI[], arr: ArrFields) {
            ...
            let arrBandUI: ArrBandUIX = {
                ...
                label: name,
                row: RowContent,
                bandUIs: undefined, // fieldBandUIs,
                band: ArrBand,
                form: this,
            };
        ...
        private buildVmArr(arr:ArrFields, arrBandUI:ArrBandUIX): VmArr {
            let ret = new VmArr(this.vmUsq, arr, arrBandUI);
        ...
        */
        ...
        this.view = observer(() => {
            //let {className, bandUIs, visibleBands} = this.ui;
            //let vBands = bandUIs;
            //if (visibleBands !== undefined && visibleBands.length > 0) vBands = visibleBands;
            let { bands, className, } = this.bands;
            return React.createElement("form", { className: className, onSubmit: this.onFormSubmit }, bands.map(v => v.render()));
        });
        this.bands = bands;
        this.values = values;
    }
