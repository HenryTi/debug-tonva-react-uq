...
export class VmMapMain extends VmMap {
    ...
    protected getSearchId(key:Field): (param:any)=>Promise<number> {
        ...
        if (kq !== undefined) {
            ...
            let query = this.vmUsq.getQuery(queryName);
            return async (param:any):Promise<number> => {
                ...
                let search = new QuerySearch(this.vmUsq, query);
                let ret = await search.result(param);
                return ret[idName].id;
            };
        }
        return async (param:any):Promise<number> => {
            let search = new TuidSearch(this.vmUsq, key._tuid);
            // 怎么把搜索关键字传进来, 还需要考虑
    ...
    */
    searchOnKey(keyField, param) {
        return __awaiter(this, void 0, void 0, function* () {
            let { _tuid } = keyField;
            let val; // = await this.vmUsq.tuidSearch(_tuid, param);
