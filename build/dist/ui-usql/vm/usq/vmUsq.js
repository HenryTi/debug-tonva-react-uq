...
export class VmUsq {
    constructor(crUsq) {
        ...
        this.vmTuidLinks = tuidArr.filter(v => this.isVisible(v)).map(v => new CrEntityLink(this.crUsq.crTuidMain(v)));
        this.vmMapLinks = mapArr.filter(v => this.isVisible(v)).map(v => new CrEntityLink(this.crUsq.crMap(v)));
        this.vmSheetLinks = sheetArr.filter(v => this.isVisible(v)).map(v => new CrEntityLink(this.crUsq.crSheet(v)));
        this.vmActionLinks = actionArr.filter(v => this.isVisible(v)).map(v => new CrEntityLink(this.crUsq.crAction(v)));
        this.vmQueryLinks = queryArr.filter(v => this.isVisible(v)).map(v => new CrEntityLink(this.crUsq.crQuery(v)));
        this.vmBookLinks = bookArr.filter(v => this.isVisible(v)).map(v => new CrEntityLink(this.crUsq.crBook(v)));
    }
    isVisible(entity) {
