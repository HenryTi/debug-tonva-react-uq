...
export interface Props extends React.HTMLProps<HTMLFormElement> {
    tag?: Function | string; // PropTypes.oneOfType([PropTypes.func,PropTypes.string,]),
    className?: string; // PropTypes.string,
    model?: object; // PropTypes.object,
    method?: 'get' | 'post'; // PropTypes.oneOf(['get', 'post']),
    onSubmit?: React.EventHandler<React.FormEvent<HTMLFormElement>>; // PropTypes.func,
    validate?: Function | any[]; // PropTypes.oneOfType([PropTypes.func,PropTypes.array,]),
    onValidSubmit?: Function; // PropTypes.func,
    onInvalidSubmit?: Function; // PropTypes.func,
    validationEvent?: //PropTypes.oneOfType([PropTypes.oneOf(['onInput', 'onChange', 'onBlur', 'onFocus',]),PropTypes.arrayOf(PropTypes.oneOf(['onInput', 'onChange', 'onBlur', 'onFocus',])),]),
      ...
      ('onInput' | 'onChange' | 'onBlur' | 'onFocus')[];
    errorMessage?: object | string | React.ReactNode; // PropTypes.oneOfType([PropTypes.object,PropTypes.string,PropTypes.node,]),
    validateOne?: any;
    validateAll?: any;
}

export default class AvForm extends InputContainer<Props> {
...
*/
  ...
  handleSubmit = async (e) => {
    ...
    (this.props.onSubmit as any)(e, errors, values);
    if (isValid) {
  ...
  handleNonFormSubmission = (event) => {
    if (((this.props.onKeyDown as any)(event)) !== false) {
  ...
  render() {
    ...
    if (Tag !== 'form' && Tag !== Form) {
      ...
      (attributes as any).onKeyDown = this.handleNonFormSubmission;
    }
  ...
  setError(inputName, error = true, errText:any = error) {
